...
for i in range(2,sheet_obj.max_row-3):
    if((str(sheet_obj["J"+str(i)].value))==str(1)):
        for k in range(i+3,sheet_obj.max_row-3):
            if(float(sheet_obj["C"+str(i)].value)>=float(sheet_obj["D"+str(k)].value) and float(sheet_obj["D"+str(i)].value)<=float(sheet_obj["D"+str(k)].value)):
                sheet_obj["P"+str(k)].value="tap"+str(i)
                for j in range(k+1,sheet_obj.max_row-3):
                    if((float(sheet_obj["D"+str(i)].value)-1.5)>float(sheet_obj["D"+str(j)].value)):
